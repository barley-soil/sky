...
echo -e "${GREEN}===========================================${NC}"
echo -e "${GREEN}      开始启用 TCP BBR 拥塞控制算法      ${NC}"
echo -e "${GREEN}===========================================${NC}"

# 1. 检查内核版本是否满足要求 (4.9+)
KERNEL_VERSION=$(uname -r | awk -F. '{print $1*100 + $2}')
if [ "$KERNEL_VERSION" -lt 409 ]; then
    echo -e "${RED}❌ 错误：当前内核版本 $(uname -r) 低于 4.9，不支持 BBR。${NC}"
    echo -e "${RED}请先升级内核再运行此脚本。${NC}"
    exit 1
fi

# 2. 修改系统配置 /etc/sysctl.conf
...
if [ "$TCP_CC" == "bbr" ] && [ "$TCP_QDISC" == "fq" ]; then
    echo -e "${GREEN}🎉 恭喜！TCP BBR 拥塞控制算法已成功启用！${NC}"
    echo "当前拥塞控制算法: $TCP_CC"
    echo "当前队列调度算法: $TCP_QDISC"
else
    echo -e "${RED}❌ 验证失败！拥塞控制算法当前为: $TCP_CC${NC}"
    echo -e "${RED}请检查脚本输出或尝试重启服务器。${NC}"
fi
