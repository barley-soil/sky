...
docker run --rm \
  ...
  billbear-cn-shanghai.cr.volces.com/base/maven:3.9.0-ibm-semeru-17-focal \
  sh -c "cp -a /code/settings.xml /root/.m2/settings.xml && cd /code ${P_PACKAGE_BEFORE:-} && mvn -T 4C clean package -Dmaven.test.skip=true ${P_PACKAGE_AFTER:-}"
