...
docker run --rm \
  ...
  billbear-cn-shanghai.cr.volces.com/base/maven:3.9-eclipse-temurin-25-noble \
...
FROM billbear-cn-shanghai.cr.volces.com/base/billbear-jdk:openj9-25
...
if [ -n "${P_COLONY:-}" ]; then
  ...
  for colony in ${P_COLONY//,/ }; do
    ...
    if [ -z "$K8S_NAMESPACE" ]; then
      kubectl --kubeconfig="$KUBECONFIG" get ns --show-labels
