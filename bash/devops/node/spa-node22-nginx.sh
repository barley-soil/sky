...
docker run --rm \
  ...
  billbear-cn-shanghai.cr.volces.com/base/billbear-node:20.15-build \
  sh -c "cd /code && ${P_PACKAGE_BEFORE:-echo ''} && npm config set registry https://registry.npmmirror.com/ && ${P_BUILD_CMD:-npm} install && ${P_BUILD_CMD:-npm} run build && ${P_PACKAGE_AFTER:-echo ''}"

# Nginx → default.conf
BUILD_WEB_PATH="${P_WEB_PATH:-/}"
BUILD_WEB_PATH="/${BUILD_WEB_PATH#/}"
BUILD_WEB_PATH="${BUILD_WEB_PATH%%*(/)}"
cat <<'EOF' | sed "s|__WEB_PATH__|$BUILD_WEB_PATH|g;" >default.conf
server {
    listen          80;
    ...
    location __WEB_PATH__/ {
        alias       /data/app/;
        index index.html;
        try_files   $uri $uri/ __WEB_PATH__/index.html;
    }
}
EOF
