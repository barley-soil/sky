...
docker run --rm \
  ...
  billbear-cn-shanghai.cr.volces.com/base/billbear-node:22.15-runtime \
...
cat <<'EOF' | sed "s|__BUILD_TIME__|$BUILD_TIME|g; s|__APPLICATION_NAME__|$BUILD_APPLICATION_NAME|g; s|__COMMIT_ID__|$BUILD_COMMIT_ID|g;" >Dockerfile
FROM billbear-cn-shanghai.cr.volces.com/base/billbear-node:22.15-runtime
